...
/*
 * This file contains the function WebRtcSpl_ComplexFFT_mips().
...
static const WebRtc_Word16 kSinTable1024_pos[] = {
  32767,  32766,  32764,  32761,  32757,  32751,  32744,  32736,
  32727,  32717,  32705,  32692,  32678,  32662,  32646,  32628,
  32609,  32588,  32567,  32544,  32520,  32495,  32468,  32441,
  32412,  32382,  32350,  32318,  32284,  32249,  32213,  32176,
 ...
 -11038, -10849, -10659, -10469, -10278, -10087,  -9895,  -9703,
  ...
  static const WebRtc_Word16 kSinTable1024_neg[] = {
      0,   -201,   -402,   -603,   -804,  -1005,  -1206,  -1406,
  -1607,  -1808,  -2009,  -2209,  -2410,  -2610,  -2811,  -3011,
  -3211,  -3411,  -3611,  -3811,  -4011,  -4210,  -4409,  -4608,
  -4807,  -5006,  -5205,  -5403,  -5601,  -5799,  -5997,  -6195,
...
int WebRtcSpl_ComplexFFT(WebRtc_Word16 frfi[], int stages, int mode)
{
    int i, l, k, istep, n, m;
    WebRtc_Word16 wr, wi;
    WebRtc_Word32 tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp;
    WebRtc_Word16 *ptr_j, *ptr_i;
    ...
    if (n > 1024)
     ...
     __asm__ volatile(
       ...
       "2:                                                                 \n\t"
        ...
        "sllv       %[tmp3],        %[m],             %[k]                 \n\t"
        "addiu      %[m],           %[m],             1                    \n\t"
        "lhx        %[wr],          %[tmp3](%[kSinTable1024_pos])          \n\t"
        "lhx        %[wi],          %[tmp3](%[kSinTable1024_neg])          \n\t"
#else
        "sllv       %[tmp3],        %[m],             %[k]                 \n\t"
        "addiu      %[m],           %[m],             1                    \n\t"
        "addu       %[ptr_i],       %[tmp3],          %[kSinTable1024_pos] \n\t"
        "addu       %[ptr_j],       %[tmp3],          %[kSinTable1024_neg] \n\t"
        "lh         %[wr],          0(%[ptr_i])                            \n\t"
        "lh         %[wi],          0(%[ptr_j])                            \n\t"
#endif
       "1:                                                                 \n\t"
        ...
        : [tmp1] "=&r" (tmp1), [tmp2] "=&r" (tmp2), [tmp3] "=&r" (tmp3), [tmp4] "=&r" (tmp4),
          [tmp5] "=&r" (tmp5), [tmp6] "=&r" (tmp6), [ptr_i] "=&r" (ptr_i), [i] "=&r" (i),
          [wi] "=&r" (wi), [wr] "=&r" (wr), [m] "=&r" (m), [istep] "=&r" (istep),
          [l] "=&r" (l), [k] "=&r" (k), [ptr_j] "=&r" (ptr_j), [tmp] "=&r" (tmp)
        : [n] "r" (n), [frfi] "r" (frfi), [kSinTable1024_pos] "r" (kSinTable1024_pos),
          [kSinTable1024_neg] "r" (kSinTable1024_neg)
...
{
    int i, l, k, istep, n, m, scale, shift;
    WebRtc_Word16 wr, wi;
    WebRtc_Word32 tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp, tempMax, round2;
    WebRtc_Word16 *ptr_j, *ptr_i;
    ...
    if (n > 1024)
     ...
     __asm__ volatile (
       ...
       "1:                                                                 \n\t"
        ...
        : [tmp1] "=&r" (tmp1), [tmp2] "=&r" (tmp2), [tmp3] "=&r" (tmp3), [tmp4] "=&r" (tmp4),
          [tmp5] "=&r" (tmp5), [tmp6] "=&r" (tmp6), [ptr_i] "=&r" (ptr_i), [i] "=&r" (i),
          [m] "=&r" (m), [tmp] "=&r" (tmp), [istep] "=&r" (istep), [wi] "=&r" (wi), [wr] "=&r" (wr),
          [l] "=&r" (l), [k] "=&r" (k), [round2] "=&r" (round2), [ptr_j] "=&r" (ptr_j),
          [shift] "=&r" (shift), [scale] "=&r" (scale), [tempMax] "=&r" (tempMax)
        : [n] "r" (n), [frfi] "r" (frfi), [kSinTable1024_pos] "r" (kSinTable1024_pos),
          [kSinTable1024] "r" (kSinTable1024)
        : "hi", "lo", "$ac1hi", "$ac1lo", "memory"
     );
